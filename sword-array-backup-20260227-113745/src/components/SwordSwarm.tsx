  ...
  useFrame(({ clock }) => {
    ...
    for (let i = 0; i < config.swordCount; i++) {
      ...
      if (gestureMode === 'GATHER' && isTracking) {
        ...
        // 凤凰阵 - 基于 Lissajous 曲线的∞符号运动
        // Lissajous 2:1 比例产生完美的8字形
        const speed = 1.8;
        const scale = 6;

        // 每把剑有相位偏移，形成连续的∞符号轨迹
        const phase = time * speed + (i / config.swordCount) * Math.PI * 2;

        // Lissajous 曲线: x = sin(2t), y = sin(t) 产生8字形
        const x = Math.sin(phase * 2) * scale;
        const y = Math.sin(phase) * scale * 0.7;
        const z = Math.sin(phase * 3) * 0.5;  // 轻微Z轴起伏增加立体感
