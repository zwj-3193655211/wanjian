...
import { useHandStore, FORMATION_META } from './store';
import './index.css';
...
function StatusIndicator() {
  ...
  const getModeText = () => {
    ...
    const meta = FORMATION_META[gestureMode];
    return `${meta.emoji} ${meta.name}`;
  };

  const getModeColor = () => {
    if (!isTracking) return '#ff6666';
    return FORMATION_META[gestureMode].color;
  };
...
function UI() {
  ...
  const getHint = () => {
    const meta = FORMATION_META[gestureMode];
    if (!isTracking) {
      return useMouse ? '🖱️ 移动鼠标控制飞剑 · 点击切换阵型' : '👋 请挥手或移动鼠标激活飞剑...';
    }
    const fingerCount = {
      GATHER: 0, DRAGON: 1, HUNTIAN: 2, PHOENIX: 3, LOTUS: 4, DAGENG: 5
    }[gestureMode];
    return `${fingerCount}指 · ${meta.name} · 点击切换`;
  };
